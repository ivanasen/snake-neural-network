...
test('crossover 2 genomes', t => {
  ...
  const crossedKeys = pool.crossOverDataKey(
    neurons1,
    neurons2,
    'bias',
    cutLocation
  ...
  const neuronsFrom = (arr1, arr2, key) => {
    return arr1.reduce((acc, neuron, i) => {
      return arr2[i][key] === neuron[key] ? acc + 1 : acc
    }, 0)
  }

  t.equal(
    neuronsFrom(crossedKeys, neurons1, 'bias'),
    cutLocation,
    'Neurons from a after crossover'
  )
  t.equal(
    neuronsFrom(crossedKeys, neurons2, 'bias'),
    neurons1.length - cutLocation,
    'Neurons from b after crossover'
  )
...
test('Mutate', t => {
  ...
  t.equal(
    ...
    true,
    'Mutate changed some network neurons bias'
  )
  t.equal(
    hasAtLeastOneDifferentKeyIn(
      newGenNetworkJSON,
    ...
    true,
    'Mutate changed some network neurons weights'
  )
  t.equal(
    newGen instanceof Genome,
    true,
    'Mutated Genome is still a Genome proto'
  )
})
...
test('Matches & Opponents', t => {
  ...
  t.deepEqual(
    match1,
    { opponent: 1, score: 12, winner: true },
    'Winner should get its score'
  )
  t.deepEqual(
    match2,
    { opponent: 0, score: 1, winner: false },
    'Loser should get its score'
  )
  ...
  t.deepEqual(
    match3,
    { opponent: 2, score: 12, winner: true },
    'P1 winning subsequent matches'
  )
  t.equal(
    pool.findOpponent(false, pool.genomes),
    0,
    'P1 is still the same Genome after 2 matches'
  )
